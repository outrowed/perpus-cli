...
static bool open_pdf_with_default_app(const string& path) {
...
static void read_book(BookManager& manager) {
    ...
    string id = prompt_required("Enter book ISBN to read: ");
    try {
    ...
    string pdfPath = manager.pdf_path_for_id(id);
    cout << "PDF path: " << pdfPath << "\n";
...
static void request_loan(Session& session, const std::string& username) {
    ...
    show_book_list(manager);
    string id = prompt_required("Enter book ISBN to request: ");
    Book book;
    try {
...
static void view_requests(const LoanRequestManager& loanManager, const BookManager& manager, const std::string& username) {
    vector<LoanRequest> requests = loanManager.requests_for_user(username);
    if (requests.empty()) {
    ...
    for (size_t i = 0; i < requests.size(); ++i) {
        ...
        cout << "#" << req.id << " [" << LoanRequestManager::status_label(req.status) << "] "
                  << title << " (" << req.bookId << ") on " << LoanRequestManager::human_date(req.requestedAt) << "\n";
    }
    println("==========================\n");
}

static string manage_account(Session& session, const string& username) {
    ...
    while (true) {
        ...
        if (choice == "1") {
            string newUsername = prompt_required("New username: ");
            ...
            string newPassword = prompt_required("New password: ");
...
static void return_book(Session& session, const std::string& username) {
    vector<LoanRequest> requests = session.loans.requests_for_user(username);
    ...
    for (size_t i = 0; i < requests.size(); ++i) {
        ...
        if (req.status == LoanStatus::Approved) {
            ...
            cout << "#" << req.id << " " << title << " (" << req.bookId << ")\n";
        }
    }
    string idText = prompt_required("Enter request ID to mark as returned: ");
    int id = 0;
    try {
        id = std::stoi(idText);
